...
import { TrendingUpIcon, ShoppingCartIcon, DollarSignIcon, PackageIcon, ArrowUpRightIcon, ArrowDownRightIcon, ClockIcon, RefreshCwIcon, PlusIcon, ShareIcon, LinkIcon, EyeIcon, SettingsIcon, CopyIcon, CheckCircleIcon } from '@/components/icons';
    ...
    return (
      <div className="space-y-6 animate-pulse">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[1,2,3,4].map(i => <div key={i} className="bg-muted rounded-xl h-28" />)}
        </div>
      </div>
    );
  }
      ...
      <div className="bg-gradient-to-r from-primary to-primary/80 rounded-2xl p-8 text-primary-foreground relative overflow-hidden">
        ...
        <div className="relative flex flex-col md:flex-row md:justify-between md:items-start gap-4">
          ...
          <button
            ...
            disabled={refreshing}
            className="p-2 bg-white/20 hover:bg-white/30 rounded-lg transition self-start"
          >
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((stat, idx) => {
          ...
          return (
            <div 
              key={idx} 
              className="bg-card border border-border rounded-xl p-5 hover:shadow-lg transition-all duration-300 hover:-translate-y-1"
            >
            ...
            {quickActions.map((action, idx) => {
              ...
              return (
                <button 
                  ...
                  className="text-left p-4 bg-muted/50 hover:bg-muted rounded-xl transition group relative overflow-hidden"
                >
                  <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${action.gradient} flex items-center justify-center shadow-md mb-3 group-hover:scale-110 transition-transform`}>
                    <Icon size={20} className="text-white" />
                  </div>
