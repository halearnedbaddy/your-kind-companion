...
const getNotificationIcon = (type: string) => {
  switch (type) {
    case 'PAYMENT_RECEIVED':
      return <CreditCardIcon className="h-4 w-4 text-[#88BDBC]" size={16} />;
    case 'ITEM_SHIPPED':
      return <TruckIcon className="h-4 w-4 text-[#254E58]" size={16} />;
    case 'DELIVERY_CONFIRMED':
      return <PackageIcon className="h-4 w-4 text-[#88BDBC]" size={16} />;
    case 'ORDER_ACCEPTED':
      return <CheckIcon className="h-4 w-4 text-[#88BDBC]" size={16} />;
    case 'DISPUTE_OPENED':
...
export function NotificationBell() {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <BellIcon className="h-5 w-5 text-[#112D32]" size={20} />
            ...
            <div className="flex items-center gap-2">
              {notifications.length > 0 && (
                <>
                  ...
                  >
                    <CheckCheckIcon className="h-4 w-4 text-[#88BDBC]" size={16} />
                  </button>
                  <button
