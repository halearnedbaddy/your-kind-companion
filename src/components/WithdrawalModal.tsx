...
export function WithdrawalModal({
  ...
  const processWithdrawal = async () => {
    ...
    try {
      ...
      handleClose();
      alert(`✅ Withdrawal processed successfully!\n\nAmount: KES ${calculateFees(parseFloat(String(withdrawalAmount)), selectedMethod).netAmount.toLocaleString()}\nMethod: ${selectedMethod?.name}\nReference: WD-${Date.now()}`);
    } catch (error) {
  ...
  return (
    <>
      <style>{`
        ...
        .withdrawal-step-dot.completed { background: #88BDBC; }
        ...
        .withdrawal-recommended-badge {
          ...
          top: -10px;
          right: 1rem;
          background: linear-gradient(135deg, #88BDBC 0%, #254E58 100%);
          color: white;
