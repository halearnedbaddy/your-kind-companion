      ...
      <div className="space-y-4">
        {disputes.length > 0 ? (
          disputes.map((dispute) => (
            <div
              key={dispute.id}
              className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden transition hover:shadow-md"
            >
              {/* Dispute Header */}
              <div 
              ...
              >
                <div className="flex items-start gap-4">
                  <div className={`p-3 rounded-lg ${
                    dispute.status === 'RESOLVED' ? 'bg-emerald-100 text-emerald-600' : 'bg-orange-100 text-orange-600'
                  }`}>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase flex items-center gap-1 ${getStatusColor(dispute.status)}`}>
                    ...
                    <h3 className="text-lg font-bold text-gray-900 mb-1">{dispute.reason?.replace('_', ' ') || 'Dispute'}</h3>
              ...
              {selectedDispute === dispute.id && (
                <div className="border-t border-gray-200 p-6 bg-gray-50 space-y-4">
                  ...
                  {dispute.status !== 'RESOLVED' && dispute.status !== 'CLOSED' && (
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:border-emerald-500"
                        onKeyDown={(e) => {
                      ...
                      <button
                        ...
                        className="px-4 py-3 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed transition"
                      >
                        {sendingMessage ? (
                  ...
                  {/* Deadline Info */}
                  {dispute.deadline && (
                    <div className="flex items-center gap-2 text-sm text-gray-500 bg-white p-3 rounded-lg border border-gray-200">
                      <ClockIcon size={16} />
          ...
          <div className="text-center py-12 bg-white rounded-xl border border-gray-200 border-dashed">
            <CheckCircleIcon className="mx-auto text-emerald-500 mb-4" size={48} />
