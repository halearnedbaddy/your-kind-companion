...
export const notifyPurchaseConfirmation = async (transaction: any) => {
  ...
  if (transaction.buyerId) {
    const message = `SWIFTLINE TRANSACTION RECEIPT
...
Transaction ID: ${transaction.id}
Date & Time: ${formatDate()}
Order Amount: ${formatCurrency(transaction.amount)}
...
export const notifyItemShipped = async (transaction: any) => {
  ...
  const message = `SWIFTLINE SHIPMENT UPDATE
...
Delivery Details:
Tracking Number: ${transaction.trackingNumber || 'N/A'}
Estimated Delivery: ${transaction.estimatedDeliveryDate ? formatDate(new Date(transaction.estimatedDeliveryDate)) : 'N/A'}

Once you receive your order:
1. Inspect the product
2. Confirm delivery in the app
